...
from rp.git.CommonSource import raft
from rp.git.CommonSource.background_remover import BackgroundRemover
from rp.r import (
    is_torch_tensor,
    is_a_matrix,
    pip_import,
...
def loop_xy(x, y, w, h, loop_x, loop_y):
    x, y = x.clone(), y.clone()
    if loop_y:
        assert loop_x, "loop_y requires loop_x to be True"
        top = y < 0
        if top.any():
            y[top] = -y[top]
            x[top] = x[top] + w // 2
...
def torch_remap_image(image, x, y, *, relative=False, interp='bilinear', add_alpha_mask=False, use_cached_meshgrid=False, loop_x=False, loop_y=False):
    """
    ...
    x, y = loop_xy(x, y, in_width, in_height, loop_x, loop_y)
...
def torch_scatter_add_image(image, x, y, *, relative=False, interp='floor', height=None, width=None, prepend_ones=False, loop_x=False, loop_y=False):
    """
...
def warp_xyωc(
    ...
    # --------------------------------------------------------------
    # Expansion branch – choose wrapped or clamped gather
    # --------------------------------------------------------------
    pre_expand[:xy]  = torch_remap_image(
        I[:xy],  * -F, relative=True, interp=interp, loop_x=loop_x, loop_y=loop_y)
    pre_expand[-ωc:] = torch_remap_image(
        I[-ωc:], * -F, relative=True, interp=interp, loop_x=loop_x, loop_y=loop_y)
    ...
    pos[x], pos[y] = loop_xy(pos[x], pos[y], w, h, loop_x, loop_y)
    ...
    scat = lambda tensor: torch_scatter_add_image(
        tensor, *scat_xy, relative=True, loop_x=loop_x, loop_y=loop_y)
